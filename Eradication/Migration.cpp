        ...
        if( m_IsEnabled && (m_Offsets.count( fromNodeID ) > 0) )
        {
            ...
            for( uint32_t ig = 0; ig < 2; ig++ )
            {
                ...
                for( uint32_t ja = 0; ja < m_AgesYears.size(); ja++ )
                {
                    ...
                    m_FileStream.read( (char*) array_id, m_DestinationsPerNode * sizeof(uint32_t) );
                    m_FileStream.read( (char*) array_rt, m_DestinationsPerNode * sizeof(double)   );
...
#define THROW_ENUM_EXCEPTION( EnumName, dataFile, key, inputValue )\
    ...
    uint32_t MigrationInfoFile::ParseMetadataForFile( const std::string& data_filepath, const std::string& idreference )
    {
        ...
        try // try-catch to catch JsonObjectDemog errors like wrong type
        {
            ...
            // -------------------------------------
            std::string file_id_reference = json[ METADATA ][ MD_ID_REFERENCE ].AsString();
            if( file_id_reference != idreference )
            {
                std::stringstream ss;
            ...
            {
                throw IncoherentConfigurationException( __FILE__, __LINE__, __FUNCTION__, 
                    "offsets_str.length() / 16", offsets_str.length() / 16, 
                    "num_nodes", num_nodes );
            }
    ...
    {
        initConfigTypeMap( "Enable_Migration_Heterogeneity",  &m_IsHeterogeneityEnabled, Enable_Migration_Heterogeneity_DESC_TEXT, "Migration_Model", "FIXED_RATE_MIGRATION,VARIABLE_RATE_MIGRATION,LEVY_FLIGHTS" );
