...
namespace Kernel
{
    ...
    public:
        ...
        virtual void ClearAllNewInfectionByStrain(void);
        virtual int CountInfections(void) const;
        virtual float GetAgeOfMostRecentInfection(void);
        virtual bool GetSusceptibleStatus(int pvType);
        virtual int GetParalysisVirusTypeMask() const;

        virtual const infection_polio_reportable_list_t *GetNewInfectionReportables() const;
        virtual const infection_polio_reportable_list_t *GetInfectionReportables(bool getNewInfectionsOnly=true) const;
