...
static const char * _module = "SimpleAnalyzer"; 
...
namespace Kernel
{
    ...
    bool SimpleAnalyzer::Distribute( INodeEventContext *pNodeEventContext, IEventCoordinator2 *pEC )
    {
        ...
        {
            // Second argument is just a placeholder.  Doing our own check on trigger condition in notifyOnEvent implementation.
            pNTIC->RegisterNodeEventObserver( this, IndividualEventTriggerType::IND_EVENT_TRIGGER_TYPES );
        }
        else
        {
            ...
            [this](IIndividualHumanEventContext *ihec)
        {
            ...
            {
                pITIC->RegisterIndividualEventObserver( this );
            }
            else
            {
    ...
    bool SimpleAnalyzer::notifyOnEvent( IIndividualHumanEventContext *context, const std::string StateChange )
    {
    ...
    {
        return IndividualEventTriggerType::pairs::lookup_key( m_trigger_condition );
    }
