function output_json = Single_Node_CreateHIVmort(json_template, input_file_m,input_file_f,outfile_name)
...
if (numel(Tmp_json.Nodes) > 1 )
    warning('Json warning More than one Node, only first node will be set, see tempalte %s', json_template);
end


Node_HIVMortality = Tmp_json.Nodes{1}.IndividualAttributes.HIVMortalityDistribution;

Node_HIVMortality.NumPopulationGroups = [2,numel(row_vals),numel(col_vals)];
Node_HIVMortality.PopulationGroups{1} = {0, 1};
Node_HIVMortality.PopulationGroups{2} = row_vals';
Node_HIVMortality.PopulationGroups{3} = col_vals';

Node_HIVMortality.AxisNames = {'gender', 'time', 'age'};
Node_HIVMortality.AxisUnits = {'male =0, female=1', 'days', 'years'};
...
for ii = 1:2 %only have two genders
    ...
    for jj =1:numel(row_vals)
        ...
        for kk =1:numel(col_vals)
         ...
         Node_HIVMortality.ResultValues{ii}{jj}{kk} =  Three_d_matx(ii,jj,kk);
         
        end
    end
end
            
       Tmp_json.Nodes{1}.IndividualAttributes.HIVMortalityDistribution = Node_HIVMortality;
