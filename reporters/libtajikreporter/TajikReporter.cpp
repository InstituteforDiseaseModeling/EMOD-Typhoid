...
static const char * _module = "TajikReporter";
...
void
TajikReporter::Initialize( unsigned int nrmSize )
{
    LOG_INFO( "TajikReporter Initialize\n" );
}


TajikReporter::TajikReporter()
{
    LOG_INFO( "TajikReporter ctor\n" );
}

void
TajikReporter::BeginTimestep()
{
    LOG_DEBUG( "BeginTimestep\n" );
}

void
TajikReporter::EndTimestep( float currentTime, float dt )
{
...
)
{
    ...
    const Kernel::NodePolio* pNC_polio = static_cast<const Kernel::NodePolio*>(pNC);

    // The following three channels are not currently being used:
    //nodeChannelMap[ _stat_pop_label ][ nodeId ]                     = pNC->GetStatPop();
    //nodeChannelMap[ _infected_label ][ nodeId ]                     = pNC->GetInfected();
    //nodeChannelMap[ _susceptibleinfected_label ][ nodeId ]          = pNC_polio->GetNewDiseaseSusceptibleInfections();
    
    // The old way ... delete soon:
    //nodeChannelMap[ _susceptibleinfectedunder5_label ][ nodeId ]    = pNC_polio->GetNewDiseaseSusceptibleInfectionsUnder5();
    //nodeChannelMap[ _susceptibleinfectedover5_label ][ nodeId ]     = pNC_polio->GetNewDiseaseSusceptibleInfectionsOver5();
...
void TajikReporter::WriteKmlData()
{
    std::string output_dir = "output/";
    // Iterate over nodeChannelMapVec, channel by channel
    for( tChannel2Node2DataMapVec::iterator it = nodeChannelMapVec.begin();
...
std::string TajikReporter::GetReportName() const
{
    return std::string("TajikReporter.csv");
}
